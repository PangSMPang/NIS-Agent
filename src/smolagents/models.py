...
class OpenAIServerModel(Model):
    ...
    def truncate_content_based_on_stop_sequences(content: str, stop_sequences: List[str]) -> str:
        """Manually truncate content based on stop sequences for models that don't support stop parameter."""
        ...
        for stop_seq in stop_sequences:
            ...
            if index != -1:
                ...
                break  # Only keep content before the first match
        return content
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # Handle o3/o4 models that don't support stop parameter
        manual_stop_sequences = None
        if 'o3' in self.model_id.lower() or 'o4' in self.model_id.lower():
            manual_stop_sequences = completion_kwargs.pop('stop', None)

        # DeepSeek reasoning models don't support tool_choice
        ...
        for attempt in range(local_max_retries):
            try:
                ...
                print(f"API 请求失败，重试 {attempt + 1}/{local_max_retries} 次，错误信息: {e}")
        ...
        # Apply manual stop sequences for o3/o4 models
        if manual_stop_sequences and message.content:
            message.content = self.truncate_content_based_on_stop_sequences(message.content, manual_stop_sequences)
