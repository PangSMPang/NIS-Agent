# EXAMPLE COMMAND: python examples/open_deep_research/run_gaia.py --concurrency 32 --run-name GAIA_NISAgent_deepseek --model-id deepseek-v4-pro
import argparse
import json
import os
import threading
...
def preprocess_file_paths(row):
    if len(row["file_name"]) > 0:
        row["file_name"] = f"data/gaia/2023/{SET}/" + row["file_name"]
    return row
...
BROWSER_CONFIG = {
    "downloads_folder": "/backup/zhangxiangxin/data/ds/download",
    "request_kwargs": {
...
def create_agent_team(model: Model, enable_task_decomposition=False):
...
def answer_single_question(example, model_id, answers_file, visual_inspection_tool, enable_task_decomposition=False):
    model_params = {
        "model_id": model_id,
    ...
    # Check for attached files and generate description only once
    prompt_use_files = ""
    ...
    # Set original problem with file information included
    original_problem_with_files = example["question"] + prompt_use_files
    agent.set_original_problem(original_problem_with_files)
...
def main():
    ...
    answers_file = f"outputs/gaia_outputs/{SET}/{args.run_name}.jsonl"
    # print(get_examples_to_answer(answers_file, eval_ds)[97:98])

    tasks_to_run = get_examples_to_answer(answers_file, eval_ds)[:]

    with ThreadPoolExecutor(max_workers=args.concurrency) as exe:
        futures = [
            exe.submit(answer_single_question, example, args.model_id, answers_file, visualizer, args.enable_task_decomposition)
            # exe.submit(answer_single_question, example, "gpt-4.1", answers_file, visualizer, args.enable_task_decomposition)
